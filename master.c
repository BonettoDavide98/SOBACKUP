		...
		for(int j = 1; j < parameters.SO_MERCI + 1; j++) {
			switch(rand() % 2) {
				case 0:
					...
					ports_shm_ptr_aval[i][a].spoildate.tv_sec += parameters.SO_MIN_VITA + (rand() % (parameters.SO_MAX_VITA - parameters.SO_MIN_VITA));
	...
	while(flag) {
		...
		switch(message.mesg_text[0]) {
			...
			case 'd':
				...
				//stop a ship for storm
				a = parameters.SO_PORTI + (rand() % parameters.SO_NAVI);
				while(num_kid_pids_navi > 0 && kill(kid_pids[a], 0) != 0) {
					a = parameters.SO_PORTI + (rand() % parameters.SO_NAVI);
				}
				kill(kid_pids[a], SIGUSR1);

				//stop a port for swell
				/*a = rand() % parameters.SO_PORTI;
				while(num_kid_pids_porti > 0 && kill(kid_pids[a], 0) != 0) {
					a = rand() % parameters.SO_PORTI;
				}
				kill(kid_pids[a], SIGUSR1);*/

				if(allrequestfulfilled == 1 || num_kid_pids_navi <= 0) {
